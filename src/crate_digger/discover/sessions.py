...
def list_session_items(db_path: Path, session_id: int) -> list[DiscoverySessionItem]:
    with connect(db_path) as conn:
        rows = conn.execute(
            """
            select i.id as item_id, i.session_id, i.candidate_id, i.position,
                   i.bucket, i.score_at_selection, i.affinity_at_selection,
                   i.reasons_at_selection, i.decision, i.decided_at,
                   c.spotify_track_id, t.title,
    ...
    return [
        DiscoverySessionItem(
            ...
            track=candidate_from_row(row),
        )
        for row in rows
    ]


def record_session_review(
    db_path: Path,
    *,
    session_id: int,
    ...
    if unmarked not in {"later", "skip"}:
        raise ValueError("Choose whether unmarked tracks stay for later or are skipped")
    ...
    for item_id, choice in choices.items():
        previous = by_id[item_id].decision
        if previous is not None and previous != (
            "keep" if choice == "keep" else "pass"
        ):
    ...
    with connect(db_path) as conn:
        ...
        for item in items:
            if item.decision is not None:
                continue
            choice = choices.get(item.item_id, unmarked)
            if choice == "later":
                continue
            ...
            conn.execute(
                ...
                (state, decision, now, item.candidate_id),
            )
            if choice == "keep" and item.track.label_id is not None:
                conn.execute(
                    """update discovery_labels
                    set interest_reason = coalesce(interest_reason, 'discovered through kept track'),
        ...
        remaining = int(
            conn.execute(
                "select count(*) from discovery_session_items where session_id = ? and decision is null",
                (session_id,),
            ).fetchone()[0]
        )
        if remaining == 0 and session.status != "completed":
            conn.execute(
                "update discovery_sessions set status = 'completed', completed_at = ? where id = ?",
                (now, session_id),
            )
    if saved:
