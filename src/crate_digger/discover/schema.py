...
def ensure_discovery_schema(conn: sqlite3.Connection) -> None:
    ...
    conn.executescript(
        ...
        create table if not exists discovery_spotify_playlist (
            ...
            snapshot_id text,
            track_uris text not null default '[]',
            updated_at text not null
        );

        create table if not exists discovery_listening_runs (
            id integer primary key autoincrement,
            session_id integer not null references discovery_sessions(id),
            playlist_uri text not null,
            state text not null check (state in ('publishing', 'active', 'ending', 'ended')),
            started_at text not null,
            ended_at text,
            base_snapshot_id text,
            target_uris text not null,
            confirmed_position integer,
            history_status text not null default 'unavailable',
            history_error text,
            foreign key (session_id) references discovery_sessions(id)
        );
        create unique index if not exists one_active_listening_run
            on discovery_listening_runs(playlist_uri)
            where state in ('publishing', 'active', 'ending');
        create table if not exists discovery_listening_run_items (
            run_id integer not null references discovery_listening_runs(id),
            item_id integer not null references discovery_session_items(id),
            position integer not null,
            track_uri text not null,
            primary key (run_id, item_id)
        );
        create table if not exists discovery_play_observations (
            run_id integer not null references discovery_listening_runs(id),
            item_id integer not null references discovery_session_items(id),
