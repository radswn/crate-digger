...
def load_source_identities(
    db_path: Path, source: str, *, ensure_schema: bool = True
) -> dict[str, str]:
    if not ensure_schema and not db_path.is_file():
        return {}
    with sqlite3.connect(db_path) as conn:
        if ensure_schema:
            ...
            conn.execute(
                ...
                where type = 'table' and name = 'library_track_identities'
                """
            ).fetchone()
            is None
        ):
...
def match_tracks(
    ...
    indexed_by_path = {track.path: track for track in indexed_tracks}
    for track in imported_tracks:
        ...
        identity_path = (source_identities or {}).get(track.source_track_id or "")
        if identity_path in indexed_by_path:
            results.append(
                _matched(
                    track, indexed_by_path[identity_path], "Stable source identity"
                )
            )
            continue
