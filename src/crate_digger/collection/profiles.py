...
def calculate_status_counts(
    ...
    with _connect(db_path) as conn:
        ...
        sync_counts = conn.execute(
            """
            select
                (select count(*) from canonical_track_metadata) as canonical_tracks,
                (select count(*) from canonical_playlists) as canonical_playlists,
                (select count(*) from library_sync_conflicts where status = 'open')
                    as open_sync_conflicts,
                (select max(started_at) from library_sync_runs) as latest_sync
            """
        ).fetchone()
    result = dict(counts) if counts is not None else {}
    if sync_counts is not None:
        result.update(dict(sync_counts))
        ...
        for match in report.matches:
            ...
            if track.source_track_id is not None:
                conn.execute(
                    ...
                    """,
                    (track.source, track.source_track_id, match.track_path),
                )
            conn.execute(
                """
                ...
                (
                    ...
                    track.source_path or match.track_path,
                    imported_at,
                ),
            )
