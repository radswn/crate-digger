...
def test_curated_positive_playlist_tracks_are_not_suggested(tmp_path):
    db_path = tmp_path / "collection.sqlite3"
    seed_track(db_path, "to-download", artist_id="a", label="Label")
    seed_track(db_path, "new", artist_id="b", label="Label")
    with connect(db_path) as conn:
        conn.execute(
            """insert into discovery_playlist_memberships
            (spotify_track_id, playlist_uri, playlist_name, source_type, indexed_at)
            values (?, ?, ?, ?, ?)""",
            (
                "to-download",
    ...
    assert [
        item.track.spotify_track_id for item in build_session(db_path, size=2).items
    ] == ["new"]
